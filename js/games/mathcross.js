...
window.Games.mathcross = (function () {
  ...
  function renderBoard() {
    ...
    root.querySelector('#mcRestart').addEventListener('click', function () {
      UI.confirm(T('새 문제'), T('지금 판을 그만두고 새 문제로 시작할까요?'), function () {
  ...
  function finish(timeUp) {
    ...
    UI.resultModal({
      title: timeUp ? T('시간이 다 되었습니다') : T('다 채우셨습니다!'),
      score: sc.total,
      headline: sc.all
        ? T('빈칸 {n}개를 모두 맞히셨습니다.', { n: S.need })
        : T('{a}칸 중 {b}칸을 채우셨습니다.', { a: S.need, b: S.filled }),
      rows: rows,
      actions: [
        { label: T('닫기') },
        { label: T('다른 게임'), onClick: function () { App.gameSwitcher('mathcross'); } },
