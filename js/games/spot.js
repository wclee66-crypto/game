...
window.Games.spot = (function () {
  ...
  var LEVELS = {
    step1:  { name: '첫걸음', step: 1, cols: 2, rows: 2, diffs: 2, limit: 300, bonus: 0,
              note: '큼직한 물건 4개 · 다른 곳 2군데' },
    step2:  { name: '가볍게', step: 2, cols: 3, rows: 2, diffs: 3, limit: 300, bonus: 0,
              note: '물건 6개 · 다른 곳 3군데' },
    easy:   { name: '쉬움',   step: 3, cols: 4, rows: 2, diffs: 3, limit: 360, bonus: 0,
              note: '물건 8개 · 다른 곳 3군데' },
    normal: { name: '보통',   step: 4, cols: 4, rows: 3, diffs: 4, limit: 420, bonus: 100,
              note: '물건 12개 · 다른 곳 4군데' },
    hard:   { name: '어려움', step: 5, cols: 5, rows: 3, diffs: 5, limit: 480, bonus: 250,
              note: '물건 15개 · 다른 곳 5군데' }
  };
  ...
  var SHAPES = {
    ...
    flower: { name: '꽃', vars: 3, draw: function (c, v) {
      var n = (v === 1) ? 5 : 6, p = '', k;
      for (k = 0; k < n; k++) {
    ...
    tree: { name: '나무', vars: 3, draw: function (c, v) {
    ...
    house: { name: '집', vars: 3, draw: function (c, v) {
    ...
    fish: { name: '물고기', vars: 3, draw: function (c, v) {
    ...
    bird: { name: '새', vars: 3, draw: function (c, v) {
    ...
    sun: { name: '해', vars: 3, draw: function (c, v) {
      var n = (v === 1) ? 6 : 8, r = '', k;
      for (k = 0; k < n; k++) {
    ...
    cloud: { name: '구름', vars: 3, draw: function (c, v) {
    ...
    apple: { name: '사과', vars: 3, draw: function (c, v) {
    ...
    star: { name: '별', vars: 3, draw: function (c, v) {
    ...
    butterfly: { name: '나비', vars: 3, draw: function (c, v) {
    ...
    cup: { name: '찻잔', vars: 3, draw: function (c, v) {
    ...
    umbrella: { name: '우산', vars: 3, draw: function (c, v) {
    ...
    mushroom: { name: '버섯', vars: 3, draw: function (c, v) {
      var dots = (v === 1) ? ''
    ...
    heart: { name: '하트', vars: 3, draw: function (c, v) {
    ...
    root.innerHTML =
      '<section class="intro">' +
        ...
        '<p class="intro__desc">두 그림을 견주어 보고<br>모양이 다른 것을 눌러 주세요.<br>' +
          ...
          '어느 쪽 그림을 눌러도 되고, 틀려도 점수가 깎이지 않습니다.</small></p>' +
        (best ? '<p class="intro__best">나의 최고 기록 <b>' + UI.comma(best.score) + '점</b></p>' : '') +
        (sess && LEVELS[sess.level]
          ? '<button class="btn btn--accent btn--big" id="spResume">이어서 하기 <small>' +
  ...
  function renderBoard() {
    ...
    root.innerHTML =
      '<section class="game spot">' +
        '<div class="hud">' +
          ...
          '<div class="hud__item"><span class="hud__lbl">남은 시간</span><b id="spTime">0:00</b></div>' +
          '<div class="hud__item"><span class="hud__lbl">찾음</span><b id="spFound">0 / 0</b></div>' +
          '<div class="hud__item"><span class="hud__lbl">힌트</span><b id="spHint">0</b></div>' +
        '</div>' +
        '<div class="sp-wrap">' +
          '<figure class="sp-panel" id="spPanelA"><figcaption class="sp-cap">첫째 그림</figcaption>' +
            svgOf(S.items, 'spSvgA') + '</figure>' +
          '<figure class="sp-panel" id="spPanelB"><figcaption class="sp-cap">둘째 그림</figcaption>' +
            svgOf(rightItems(S.items, S.diffs), 'spSvgB') + '</figure>' +
        '</div>' +
        '<div class="sp-dots" id="spDots"></div>' +
        '<p class="sp-note" id="spNote">모양이 다른 곳 ' + S.diffs.length + '군데를 찾아 눌러 보세요.</p>' +
        '<div class="tools" id="spTools">' +
          '<button class="tool" id="spHintBtn"><span>💡</span>힌트</button>' +
          '<button class="tool" id="spRestart"><span>↺</span>새 판</button>' +
          '<button class="tool" id="spSwitch"><span>⇄</span>다른 게임</button>' +
        '</div>' +
      '</section>';
    ...
    root.querySelector('#spRestart').addEventListener('click', function () {
      UI.confirm('새 판', '지금 판을 그만두고 새 그림으로 시작할까요?', function () {
  ...
  function foundOne(k) {
    ...
    els.note.textContent = '찾았습니다! ' + (S.diffs.length - S.found.length) + '군데 남았습니다.';
  }
  ...
  function missOne(side, p) {
    ...
    els.note.textContent = '이곳은 두 그림이 같습니다. 다시 살펴보세요.';
  }
  ...
  function useHint() {
    ...
    if (!rest.length) { UI.toast('표시할 곳이 더 없습니다.'); return; }
    ...
    paintMarks(false);
    persist();
    UI.toast('한 곳을 노란 동그라미로 표시했습니다. (−50점)', 2400);
  }
  ...
  function finish(timeUp) {
    ...
    var L = lv(), sc = score();
    Store.addRecord({
      game: 'spot', score: sc.total, difficulty: L.step + '단계 ' + L.name, duration: S.elapsed,
      detail: { found: S.found.length, total: S.diffs.length, wrong: S.wrong, hints: S.hints, timeUp: !!timeUp }
    });

    var rows = [{ label: '찾은 곳 (' + S.found.length + '/' + S.diffs.length + '군데)', value: sc.find }];
    if (sc.all) rows.push({ label: '시간 보너스 (' + UI.fmtTime(Math.max(0, L.limit - S.elapsed)) + ' 남김)', value: sc.time });
    rows.push({ label: '정확도 보너스 (헛짚음 ' + S.wrong + '회)', value: sc.acc });
    ...
    var missed = [];
    S.diffs.forEach(function (d, k) {
      if (S.found.indexOf(k) < 0) missed.push(KIND_NAME[d.kind] || '달라진 곳');
    });

    UI.resultModal({
      title: timeUp ? '시간이 다 되었습니다' : '다 찾으셨습니다!',
      score: sc.total,
      headline: sc.all
        ? '다른 곳 ' + S.diffs.length + '군데를 모두 찾으셨습니다.'
        : '못 찾으신 곳: ' + missed.join(', '),
      rows: rows,
      note: sc.all ? '' : '「그림 보기」를 누르시면 못 찾은 곳을 점선 동그라미로 알려 드립니다.',
      actions: [
        { label: '그림 보기' },
        { label: '다른 게임', onClick: function () { App.gameSwitcher('spot'); } },
  ...
  function afterTools() {
    ...
    if (!t) return;
    t.innerHTML =
      '<button class="tool" id="spAgain"><span>↺</span>한 판 더</button>' +
      '<button class="tool" id="spSwitch2"><span>⇄</span>다른 게임</button>';
    ...
    if (els.note) els.note.textContent = '초록 동그라미는 찾으신 곳, 점선 동그라미는 못 찾으신 곳입니다.';
  }
  ...
  return {
    ...
    rules: {
      title: '틀린그림찾기 점수 규칙',
      lines: [
        ['난이도', '1단계 물건 4개·2군데 · 2단계 6개·3군데 · 3단계 8개·3군데 · 4단계 12개·4군데 · 5단계 15개·5군데'],
        ['찾기 점수', '최대 600점 · 찾은 곳 수에 비례 (모두 찾으면 600점)'],
        ['시간 보너스', '최대 300점 · 모두 찾았을 때만, 남은 시간에 비례'],
        ['정확도 보너스', '최대 100점 · 엉뚱한 곳을 1번 누를 때마다 10점씩 줄어듦'],
        ['힌트 감점', '힌트 1회마다 50점 차감 (한 곳을 노란 동그라미로 알려 줍니다)'],
        ['달라지는 방식', '모양이 달라짐 · 색이 바뀜 · 다른 물건으로 바뀜, 세 가지뿐입니다'],
        ['모양의 차이', '물건마다 정해져 있습니다 — 우산은 손잡이 방향, 물고기는 눈 색과 보는 방향, 하트는 뒤집힘, 별은 살짝 기울어짐, 집은 굴뚝, 해는 빛살 수처럼 자세히 보면 확실한 것만 냅니다'],
        ['두 그림의 물건 수', '항상 똑같습니다. 한쪽에만 있는 물건이나 크기·자리 차이는 내지 않습니다'],
        ['난이도 보너스', '보통 +100점, 어려움 +250점 (모두 찾았을 때)'],
        ['최고 점수', '1~3단계 1,000점 / 보통 1,100점 / 어려움 1,250점'],
        ['시간이 끝나면', '찾은 만큼만 점수로 기록되고, 못 찾은 곳을 알려 드립니다']
      ]
    },
