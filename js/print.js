...
window.Print = (function () {
  ...
  function sheetHead(title, level, sub) {
    return '<div class="ps-head">' +
      ...
      '</div>' +
      ('<div class="ps-name">' + T('이름') + ' <span class="ps-blank"></span> ' + T('날짜') + ' <span class="ps-blank ps-blank--sm"></span></div>') +
    '</div>' +
  ...
  function sudokuSheets(o) {
    ...
    for (var n = 0; n < o.count; n++) {
      ...
      pages.push('<section class="ps-sheet">' +
        sheetHead(T('스도쿠'), made.levelName + (o.count > 1 ? ' · ' + T('{n}번', { n: n + 1 }) : ''),
      ...
      if (o.answer) {
        pages.push('<section class="ps-sheet ps-sheet--ans">' +
          sheetHead(T('스도쿠 정답'), made.levelName + (o.count > 1 ? ' · ' + T('{n}번', { n: n + 1 }) : ''), '') +
  ...
  function wordsearchSheets(o) {
    ...
    for (var n = 0; n < o.count; n++) {
      ...
      if (o.answer) {
        ...
        pages.push('<section class="ps-sheet ps-sheet--ans">' +
          sheetHead(T('낱말찾기 정답'), (T('주제') + ' ') + b.theme + (o.count > 1 ? ' · ' + T('{n}번', { n: n + 1 }) : ''), '') +
          wsWords(b) +
          wsGrid(b, marked) +
        '</section>');
      }
  ...
  function mathSheets(o) {
    ...
    for (var n = 0; n < o.count; n++) {
      ...
      pages.push('<section class="ps-sheet ps-sheet--left">' +
        sheetHead(T('숫자 계산'), b.levelName + (o.count > 1 ? ' · ' + T('{n}번', { n: n + 1 }) : ''),
          T('빈칸에 답을 적으세요.')) +
        body +
      '</section>');
  ...
  function wordorderSheets(o) {
    ...
    for (var n = 0; n < o.count; n++) {
      ...
      pages.push('<section class="ps-sheet">' +
        sheetHead(T('단어 순서 바로잡기'), b.levelName + (o.count > 1 ? ' · ' + T('{n}번', { n: n + 1 }) : ''),
      ...
      if (o.answer) {
        pages.push('<section class="ps-sheet ps-sheet--ans">' +
          sheetHead(T('단어 순서 정답'), b.levelName + (o.count > 1 ? ' · ' + T('{n}번', { n: n + 1 }) : ''), '') +
          woRows(b.items, true) +
        '</section>');
      }
    }
    ...
    for (var n = 0; n < o.count; n++) {
      ...
      pages.push('<section class="ps-sheet">' +
        sheetHead(T('틀린그림찾기'), b.levelName + no,
          T('두 그림을 견주어 보고, 모양이 다른 곳 {n}군데를 찾아 동그라미를 치세요.', { n: b.count })) +
        spotPanel(b, b.first, '', T('첫째 그림')) +
        spotPanel(b, b.second, '', T('둘째 그림')) +
      '</section>');

      if (o.answer) {
        pages.push('<section class="ps-sheet ps-sheet--ans">' +
          sheetHead(T('틀린그림찾기 정답'), b.levelName + no, '') +
          spotPanel(b, b.first, b.marksA, T('첫째 그림')) +
          spotPanel(b, b.second, b.marksB, T('둘째 그림')) +
        '</section>');
      }
    }
    return pages.join('');
  ...
  function run(game, o) {
    ...
    if (!make) {                                  // 인쇄 방법을 아직 안 만든 게임
      UI.toast(T('이 게임은 아직 종이로 만들 수 없습니다.'));
      return;
    }
    printHtml(make(o));
  }
  ...
  function dialog(game) {
    ...
    var picItems = cl
      ? [['random', T('아무거나')]].concat(PICTURES.list.map(function (p) { return [p.id, p.name]; }))
      : null;
    ...
    var m = UI.modal({
      ...
      actions: [
        { label: T('취소') },
        { label: T('만들기'), kind: 'accent', onClick: function () { run(game, pick); } }
      ]
    });
