...
window.UI = (function () {
  ...
  function beep(kind) {
    ...
    try {
      ...
      /* 넷째 자리는 소리의 결(파형)이다. 적지 않으면 예전처럼 sine 이다.
         맞았을 때 나는 소리 둘을 밝은 종소리(triangle)로 바꿨다 —
           ok   한 문제를 맞혔을 때 (숫자 계산·그림자 맞추기·상식 퀴즈 …) — 「딩 동 댕」
           tick 한 칸을 바르게 채웠을 때 (스도쿠·숫자 찾기·계산 퍼즐 …) — 한 번 울리는 맑은 종소리
         ok 는 도·미·높은도 세 음이다. 음 사이를 0.14초씩 벌려야 세 음절로 또렷하게 들린다.
         (붙여 놓으면 「띠리링」 한 덩어리로 들려 딩동댕이 되지 않는다 — 2026-09-13 에 골라 정했다)
         마지막 음만 한 옥타브 위로 올리고 길게 울려 「댕!」 하는 맺음을 만든다.
         tick 은 한 판에 수십 번 울리므로 일부러 짧고 한 음으로 둔다. 길면 성가시다.
         오답(no)·완료(win) 소리는 그대로 두었다. */
      var seq = {
        ok: [[523, 0, 0.16, 'triangle'], [659, 0.14, 0.16, 'triangle'], [1047, 0.28, 0.50, 'triangle']],
        no: [[196, 0, 0.18]],
        tick: [[988, 0, 0.055, 'triangle']],
